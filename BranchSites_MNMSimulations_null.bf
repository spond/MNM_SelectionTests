...
outfile_path = filepath + ".null.includingdelta.out";
...
global omega_FG := ((site_kind==1)*omega_0+(site_kind==2)+(site_kind>2)); 
...
{
	sim_outfile_path = filepath + ".null.SimSeq." + simCounter;
	...
	fprintf(sim_outfile_path, filtered_simulatedData);
	fprintf(sim_outfile_path, simulatedTree);
}
